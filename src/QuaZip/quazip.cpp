...
bool QuaZip::open(Mode mode, zlib_filefunc_def* ioApi)
{
  p->zipError=UNZ_OK;
  if(isOpen()) {
    qDebug("QuaZip::open(): ZIP already opened");
    return false;
  }
  QIODevice *ioDevice = p->ioDevice;
  if (ioDevice == NULL) {
    if (p->zipName.isEmpty()) {
      qDebug("QuaZip::open(): set either ZIP file name or IO device first");
      return false;
    } else {
  ...
  switch(mode) {
    ...
    default:
      qDebug("QuaZip::open(): unknown mode: %d", (int)mode);
      if (!p->zipName.isEmpty())
        delete ioDevice;
      return false;
      break;
...
void QuaZip::close()
{
  ...
  switch(p->mode) {
    case mdNotOpen:
      qDebug("QuaZip::close(): ZIP is not open");
      return;
    case mdUnzip:
    ...
    case mdAdd:
      ...
      break;
    default:
      qDebug("QuaZip::close(): unknown mode: %d", (int)p->mode);
      return;
  }
...
{
  if(isOpen()) {
    qDebug("QuaZip::setZipName(): ZIP is already open!");
    return;
  }
  p->zipName=zipName;
  p->ioDevice = NULL;
...
{
  if(isOpen()) {
    qDebug("QuaZip::setIoDevice(): ZIP is already open!");
    return;
  }
  p->ioDevice = ioDevice;
  p->zipName = QString();
...
int QuaZip::getEntriesCount()const
{
  ...
  if(p->mode!=mdUnzip) {
    qDebug("QuaZip::getEntriesCount(): ZIP is not open in mdUnzip mode");
    return -1;
  }
...
QString QuaZip::getComment()const
{
  ...
  fakeThis->p->zipError=UNZ_OK;
  if(p->mode!=mdUnzip) {
    qDebug("QuaZip::getComment(): ZIP is not open in mdUnzip mode");
    return QString();
  }
  unz_global_info globalInfo;
  QByteArray comment;
...
bool QuaZip::setCurrentFile(const QString& fileName, CaseSensitivity cs)
{
  p->zipError=UNZ_OK;
  if(p->mode!=mdUnzip) {
    qDebug("QuaZip::setCurrentFile(): ZIP is not open in mdUnzip mode");
    return false;
  }
  if(fileName.isEmpty()) {
...
bool QuaZip::goToFirstFile()
{
  p->zipError=UNZ_OK;
  if(p->mode!=mdUnzip) {
    qDebug("QuaZip::goToFirstFile(): ZIP is not open in mdUnzip mode");
    return false;
  }
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo *info)const
{
  ...
  fakeThis->p->zipError=UNZ_OK;
  if(p->mode!=mdUnzip) {
    qDebug("QuaZip::getCurrentFileInfo(): ZIP is not open in mdUnzip mode");
    return false;
  }
...
QString QuaZip::getCurrentFileName()const
{
  ...
  fakeThis->p->zipError=UNZ_OK;
  if(p->mode!=mdUnzip) {
    qDebug("QuaZip::getCurrentFileName(): ZIP is not open in mdUnzip mode");
    return QString();
  }
...
bool QuaZipPrivate::getFileInfoList(QList<TFileInfo> *result) const
{
  ...
  if (mode!=QuaZip::mdUnzip) {
    qDebug("QuaZip::getFileNameList/getFileInfoList(): "
